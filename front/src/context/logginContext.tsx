...
const LogginContext = createContext<ILogginContext | undefined>(undefined);

export const useLoggin = () => {
  const context = useContext(LogginContext);
  if (!context) {
    throw new Error('useLoggin debe ser usado dentro de un LogginProvider');
  }
  return context;
};

export const LogginProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
